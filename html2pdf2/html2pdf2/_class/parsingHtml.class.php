...
class HTML2PDF_parsingHtml
{
    ...
    public function parse()
    {
                    ...
                    if (!in_array($res['name'], $tagsNotClosed)) {
                        ...
                        if ($res['close']) {
                            ...
                            // else :add a child for validation
                                ...
                                $parents[count((array) $parents)] = $res['name'];
                        }
        ...
        if (count((array) $parents)) throw new HTML2PDF_exception(5, $parents);
    ...
    protected function _analiseCode($code)
    {
        ...
        if (in_array($name, array('ul', 'ol', 'table')) && !$close) {
            $this->_num++;
            $this->_level[count((array) $this->_level)] = $this->_num;
        }

        // get the level of the table containing the element
        if (!isset($param['num'])) {
            $param['num'] = $this->_level[count((array) $this->_level)-1];
        }

        // for closures table: remove a level
        if (in_array($name, array('ul', 'ol', 'table')) && $close) {
            unset($this->_level[count((array) $this->_level)-1]);
        }
