...
class HTML2PDF_parsingCss
{
    ...
    {
        return isset($this->table[count((array) $this->table)-1]) ? $this->table[count((array) $this->table)-1] : $this->value;
    }
    ...
    */
    ...
    public function getSvgStyle($tagName, &$param)
    {
        ...
        if (count((array) $class)) {
            foreach ((array) $class as $v) {
                $this->value['id_lst'][] = '*.'.$v;
                $this->value['id_lst'][] = '.'.$v;
    ...
    public function analyse($tagName, &$param, $legacy = null)
    {
        ...
        if ($legacy) {
            foreach ((array) $legacy as $legacyName => $legacyValue) {
        ...
        foreach ((array) $styles as $nom => $val) {
            switch($nom)
            {
                case 'font-family':
                    $val = explode(',', $val);
                ...
                case 'border-style':
                    $val = explode(' ', $val);
                    foreach ((array) $val as $valK => $valV) {
                ...
                case 'border-radius':
                    $val = explode('/', $val);
                    if (count((array) $val)>2) {
                        break;
                    }
                    $valH = $this->convertToRadius(trim($val[0]));
                    if (count((array) $valH)<1 || count((array) $valH)>4) {
                        break;
                    }
                    ...
                    if (isset($val[1])) {
                        ...
                        if (count((array) $valV)<1 || count((array) $valV)>4) {
                            break;
                        }
                ...
                case 'border-top-left-radius':
                    ...
                    if (count((array) $val)<1 || count((array) $val)>2) {
                        break;
                    }
    ...
    {
        for ($k=count((array) $this->table)-1; $k>=0; $k--) {
    ...
    protected function _getFromCSS()
    {
        ...
        if (count((array) $getit)) {
            // get them, but in the definition order, because of priority
            asort($getit);
            foreach ((array) $getit as $key => $val) $styles = array_merge($styles, $this->css[$key]);
        }
    ...
    protected function _duplicateBorder(&$val)
    {
        ...
        if (count((array) $val)==1) {
            ...
            $val[3] = $val[0];
        // 2 values => L => R & T => B
        } else if (count((array) $val)==2) {
            $val[2] = $val[0];
            $val[3] = $val[1];
        // 3 values => T => B
        } else if (count((array) $val)==3) {
            $val[3] = $val[1];
        }
    }
    ...
    public function convertBackgroundPosition($css, &$res)
    {
        ...
        if (count((array) $css)<2) {
            if (!$css[0]) return null;
            $css[1] = 'center';
        }
        if (count((array) $css)>2) return null;
        ...
        foreach ((array) $match[1] as $code) {
            ...
            if (isset($tmp['type']) && strtolower($tmp['type'])=='text/css' && isset($tmp['href'])) {
                ...
                if (strpos($url, 'http://')!==false) {
                    ...
                    $urlSelf = $url; unset($urlSelf[count((array) $urlSelf)-1]); $urlSelf = 'http://'.implode('/', $urlSelf).'/';
