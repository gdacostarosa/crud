...
if (!class_exists('QRcode', false)) {
	...
	class QRcode {
		...
		public function __construct($code, $eclevel = 'L') {
			...
			$size = count((array) $qrTab);
			$barcode_array['num_rows'] = $size;
			$barcode_array['num_cols'] = $size;
			$barcode_array['bcode'] = array();
			foreach ((array) $qrTab as $line) {
				$arrAdd = array();
		...
		protected function binarize($frame) {
			$len = count((array) $frame);
			// the frame is square (width = height)
			foreach ((array) $frame as &$frameLine) {
				for ($i=0; $i<$len; $i++) {
		 ...
		 protected function mask($width, $frame, $level) {
			...
			if (QR_FIND_FROM_RANDOM !== false) {
				...
				for ($i = 0; $i <  $howManuOut; ++$i) {
					$remPos = rand (0, count((array) $checked_masks)-1);
					unset($checked_masks[$remPos]);
					$checked_masks = array_values($checked_masks);
				}
			}
			$bestMask = $frame;
			foreach ((array) $checked_masks as $i) {
		 ...
		 */
		 protected function newInputItem($mode, $size, $data, $bstream=null) {
			...
			if (count((array) $setData) < $size) {
				$setData = array_merge($setData, array_fill(0, ($size - count((array) $setData)), 0));
			}
		 ...
		 */
		 protected function createBitStream($items) {
			...
			foreach ((array) $items as $key => $item) {
				...
				$bits = count((array) $items[$key]['bstream']);
				$total += $bits;
			}
		 ...
		 */
		 protected function appendBitstream($bitstream, $append) {
			if ((!is_array($append)) OR (count((array) $append) == 0)) {
				return $bitstream;
			}
			if (count((array) $bitstream) == 0) {
				return $append;
			}
		...
		* Return block number 1
		 ...
		 */
		 protected function init_rs($symsize, $gfpoly, $fcr, $prim, $nroots, $pad) {
			foreach ((array) $this->rsitems as $rs) {
