...
if (!defined('__CLASS_HTML2PDF__')) {
    ...
    class HTML2PDF
    {
         ...
         */
        public function __construct($orientation = 'P', $format = 'A4', $langue='pt', $unicode=true, $encoding='UTF-8', $marges = array(5, 5, 5, 8))
        { 
        ...
        public function getHtmlFromPage($html)
        {
            ...
            if (count((array) $res)<2) return $html;
            $content = '<page'.$res[1];
...
'.$content.'
        ...
        protected function _getMargins($y)
        {
            ...
            foreach ((array) $this->_pageMarges as $mY => $mX)
                if ($mY<=$y) $x = $mX;
        ...
        {
            if (!count((array) $this->_subHEADER)) return false;
        ...
        {
            if (!count((array) $this->_subFOOTER)) return false;
        ...
        protected function _setNewPositionForNewLine($curr = null)
        {
            ...
            $res = null;
            for ($sub->_parsePos; $sub->_parsePos<count((array) $sub->parsingHtml->code); $sub->_parsePos++) {
        ...
        {
            $this->_defList[count((array) $this->_defList)-1]['nb']++;
        }
        ...
        {
            $im = $this->_defList[count((array) $this->_defList)-1]['img'];
            $st = $this->_defList[count((array) $this->_defList)-1]['style'];
            $nb = $this->_defList[count((array) $this->_defList)-1]['nb'];
        ...
        protected function _listeAddLevel($type = 'ul', $style = '', $img = null)
        {
            ...
            // add the new level
            $this->_defList[count((array) $this->_defList)] = array('style' => $style, 'nb' => 0, 'img' => $img);
        }
        ...
        {
            if (count((array) $this->_defList)) {
                unset($this->_defList[count((array) $this->_defList)-1]);
        ...
        protected function _makeHTMLcode()
        {
            ...
            for ($this->_parsePos=0; $this->_parsePos<count((array) $this->parsingHtml->code); $this->_parsePos++) {
        ...
        protected function _prepareTransform($transform)
        {
            ...
            // for actions
            for ($k=0; $k<count((array) $match[0]); $k++) {
        ...
        protected function _calculateTableCellSize(&$cases, &$corr)
        {
            ...
            $sw = array();
            for ($x=0; $x<count((array) $corr[0]); $x++) {
                $m=0;
                for ($y=0; $y<count((array) $corr); $y++) {
        ...
        protected function _tag_open_PAGE($param)
        {
            ...
            if ($newPageSet) {
                ...
                if (isset($param['footer'])) {
                    ...
                    foreach ((array) $lst as $key => $val) $lst[$key] = trim(strtolower($val));
        ...
        protected function _tag_open_PAGE_HEADER($param)
        {
            ...
            $this->_subHEADER = array();
            for ($this->_parsePos; $this->_parsePos<count((array) $this->parsingHtml->code); $this->_parsePos++) {
                $action = $this->parsingHtml->code[$this->_parsePos];
        ...
        protected function _tag_open_FIELDSET($param)
        {
            ...
            for ($tempPos = $this->_parsePos + 1; $tempPos<count((array) $this->parsingHtml->code); $tempPos++) {
                ...
                if ($action['name'] == 'legend' && !$action['close']) {
                    ...
                    $res = null;
                    for ($sub->_parsePos = 0; $sub->_parsePos<count((array) $sub->parsingHtml->code); $sub->_parsePos++) {
                        $action = $sub->parsingHtml->code[$sub->_parsePos];
        ...
        protected function _tag_open_WRITE($param)
        {
            ...
            while ($x+$w>$right && $x<$right+$space && count((array) $words)) {
                ...
                if (count((array) $words)) {
                    ...
                    $nb++;
                    if ($nb>10000) {
                        $txt = ''; foreach ((array) $words as $k => $word) $txt.= ($k ? ' ' : '').$word[0];
        ...
        protected function _tag_open_UL($param, $other = 'ul')
        {
            ...
            if (!in_array($this->_previousCall, array('_tag_close_P', '_tag_close_UL'))) {
                if ($this->_maxH) $this->_tag_open_BR(array());
                if (!count((array) $this->_defList)) $this->_tag_open_BR(array());
            }
        ...
        protected function _tag_open_THEAD($param)
        {
            ...
            if ($this->_subPart) {
                ...
                for ($pos=$this->_tempPos; $pos<count((array) $this->parsingHtml->code); $pos++) {
                ...
                $level = $this->parsingHtml->getLevel($this->_parsePos);
                $this->_parsePos+= count((array) $level);
        ...
        protected function _tag_open_TR($param, $other = 'tr')
        {
            ...
            if (!$this->_subPart) {
                ...
                // Y after the row
                $ty=null;
                for ($ii=0; $ii<count((array) HTML2PDF::$_tables[$param['num']]['cases'][HTML2PDF::$_tables[$param['num']]['tr_curr']-1]); $ii++) {
        ...
        protected function _tag_open_TD($param, $other = 'td')
        {
            ...
            if (in_array($other, array('td', 'th'))) {
                ...
                if (isset(HTML2PDF::$_tables[$param['num']]['cols'][$numCol])) {
                    ...
                    if (count((array) $colParam['style']['width'])) {
                        $total = $colParam['style']['width'][0]; unset($colParam['style']['width'][0]);
                        foreach ((array) $colParam['style']['width'] as $width) {
            ...
            if ($this->_subPart) {
                ...
                $this->_tempPos+= count((array) $level);
            } else {
                // new position in the table
                HTML2PDF::$_tables[$param['num']]['td_curr']++;
        ...
        {
            if (!isset($param['name'])) {
                $param['name'] = 'champs_pdf_'.(count((array) $this->_lstField)+1);
            }
        ...
        protected function _tag_open_OPTION($param)
        {
            ...
            $level = $this->parsingHtml->getLevel($this->_parsePos);
            $this->_parsePos+= count((array) $level);
            $value = isset($param['value']) ? $param['value'] : 'aut_tag_open_opt_'.(count((array) $this->_lstSelect)+1);
        ...
        {
            if (!isset($param['name']))  $param['name']  = 'champs_pdf_'.(count((array) $this->_lstField)+1);
            if (!isset($param['value'])) $param['value'] = '';
        ...
        protected function _tag_open_POLYLINE($param)
        {
            ...
            if ($path) {
                ...
                $actions = array();
                for ($k=0; $k<count((array) $path); $k+=2) {
                    $actions[] = array(
                        ($k ? 'L' : 'M') ,
        ...
        protected function _tag_open_PATH($param)
        {
            ...
            if ($path) {
                ...
                for ($k=0; $k<count((array) $path);true) {
