	...
	class TCPDF {
		...
		public function AddSpotColor($name, $c, $m, $y, $k) {
			if (!isset($this->spot_colors[$name])) {
				$i = 1 + count((array) $this->spot_colors);
		...
		public function GetNumChars($s) {
			if (($this->CurrentFont['type'] == 'TrueTypeUnicode') OR ($this->CurrentFont['type'] == 'cidfont0')) {
				return count((array) $this->UTF8StringToArray($s));
			}
			return strlen($s);
		}
		...
		public function Annotation($x, $y, $w, $h, $text, $opt=array('Subtype'=>'Text'), $spaces=0) {
			...
			if (isset($this->transfmatrix)) {
				for ($i=$this->transfmatrix_key; $i > 0; --$i) {
					$maxid = count((array) $this->transfmatrix[$i]) - 1;
					for ($j=$maxid; $j >= 0; --$j) {
			...
			if ((($opt['Subtype'] == 'FileAttachment') OR ($opt['Subtype'] == 'Sound')) AND (!$this->empty_string($opt['FS'])) AND file_exists($opt['FS']) AND (!isset($this->embeddedfiles[basename($opt['FS'])]))) {
				$this->embeddedfiles[basename($opt['FS'])] = array('file' => $opt['FS'], 'n' => (count((array) $this->embeddedfiles) + $this->embedded_start_obj_id));
			}
		...
		protected function _putEmbeddedFiles() {
			...
			foreach ((array) $this->embeddedfiles as $filename => $filedata) {
		...
		protected function getCellCode($w, $h=0, $txt='', $border=0, $ln=0, $align='', $fill=0, $link='', $stretch=0, $ignore_min_height=false, $calign='T', $valign='M') {
			...
			if ($txt != '') {
				...
				if ($this->isunicode) {
					if (($this->CurrentFont['type'] == 'core') OR ($this->CurrentFont['type'] == 'TrueType') OR ($this->CurrentFont['type'] == 'Type1')) {
						...
						if (defined('K_THAI_TOPCHARS') AND (K_THAI_TOPCHARS == true)) {
							...
							$numchars = count((array) $unicode); // number of chars
							$unik = 0;
							$uniblock = array();
							$uniblock[$unik] = array();
				...
				if (isset($uniblock)) {
					...
					foreach ((array) $uniblock as $uk => $uniarr) {
						if (($uk % 2) == 0) {
							// x space to skip
							if ($spacewidth != 0) {
								// justification shift
		...
		public function getNumLines($txt, $w=0) {
			...
			$txtblocks = explode("\n", $txt);
			// for each text block
			foreach ((array) $txtblocks as $block) {
		...
		public function getImageFileType($imgfile, $iminfo=array()) {
			if (isset($iminfo['mime']) AND !empty($iminfo['mime'])) {
				...
				if ((count((array) $mime) > 1) AND ($mime[0] == 'image') AND (!empty($mime[1]))) {
					return trim($mime[1]);
				}
			}
			$type = '';
		...
		public function Image($file, $x='', $y='', $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='', $ismask=false, $imgmask=false, $border=0, $fitbox=false, $hidden=false, $fitonpage=false) {
			...
			$neww = round((float) $w * $this->k * $dpi / $this->dpi);
			$newh = round((float) $h * $this->k * $dpi / $this->dpi);
		...
		protected function _putpages() {
			...
			for ($n=1; $n <= $nb; ++$n) {
				...
				if (!empty($this->pagegroups)) {
					...
					++$pagegroupnum;
					foreach ((array) $this->pagegroups as $k => $v) {
		...
		protected function _putannotsrefs($n) {
			...
			if (isset($this->PageAnnots[$n])) {
				$num_annots = count((array) $this->PageAnnots[$n]);
		...
		protected function _putannotsobjs() {
			...
			for ($n=1; $n <= $this->numpages; ++$n) {
				if (isset($this->PageAnnots[$n])) {
					...
					foreach ((array) $this->PageAnnots[$n] as $key => $pl) {
						...
						if (isset($this->radiobutton_groups[$n][$pl['txt']]) AND is_array($this->radiobutton_groups[$n][$pl['txt']])) {
							...
							$annots .= ' /Ff 49152';
							$annots .= ' /Kids [';
							foreach ((array) $this->radiobutton_groups[$n][$pl['txt']] as $data) {
								$annots .= ' '.$data['kid'].' 0 R';
								if ($data['def'] !== 'Off') {
									$defval = $data['def'];
								}
						...
						if (isset($pl['opt']['ap'])) {
							...
							if (is_array($pl['opt']['ap'])) {
								foreach ((array) $pl['opt']['ap'] as $apmode => $apdef) {
									// $apmode can be: n = normal; r = rollover; d = down;
									$annots .= ' /'.strtoupper($apmode);
									if (is_array($apdef)) {
										$annots .= ' <<';
										foreach ((array) $apdef as $apstate => $stream) {
						...
						if (isset($pl['opt']['bs']) AND (is_array($pl['opt']['bs']))) {
							...
							if (isset($pl['opt']['bs']['d']) AND (is_array($pl['opt']['bs']['d']))) {
								$annots .= ' /D [';
								foreach ((array) $pl['opt']['bs']['d'] as $cord) {
									$annots .= ' '.intval($cord);
								}
								$annots .= ']';
							}
							...
							$annots .= ' /Border [';
							if (isset($pl['opt']['border']) AND (count((array) $pl['opt']['border']) >= 3)) {
								...
								if (isset($pl['opt']['border'][3]) AND is_array($pl['opt']['border'][3])) {
									$annots .= ' [';
									foreach ((array) $pl['opt']['border'][3] as $dash) {
										$annots .= intval($dash).' ';
									}
									$annots .= ']';
								}
							...
							case 'widget': {
								...
								if (isset($pl['opt']['v'])) {
									...
									if (is_array($pl['opt']['v'])) {
										foreach ((array) $pl['opt']['v'] AS $optval) {
											if (is_float($optval)) {
								...
								if (isset($pl['opt']['dv'])) {
									...
									if (is_array($pl['opt']['dv'])) {
										foreach ((array) $pl['opt']['dv'] AS $optval) {
											if (is_float($optval)) {
												$optval = sprintf('%.2F', $optval);
											}
											$annots .= ' '.$optval;
								...
								if (isset($pl['opt']['rv'])) {
									...
									if (is_array($pl['opt']['rv'])) {
										foreach ((array) $pl['opt']['rv'] AS $optval) {
											if (is_float($optval)) {
												$optval = sprintf('%.2F', $optval);
											}
											$annots .= ' '.$optval;
		...
		protected function _putfonts() {
			$nf = $this->n;
			foreach ((array) $this->diffs as $diff) {
				//Encodings
				$this->_newobj();
			...
			$mqr = $this->get_mqr();
			$this->set_mqr(false);
			foreach ((array) $this->FontFiles as $file => $info) {
			...
			$this->set_mqr($mqr);
			foreach ((array) $this->fontkeys as $k) {
				//Font objects
				...
				if ($type == 'core') {
					...
					foreach ((array) $font['desc'] as $fdk => $fdv) {
						if(is_float($fdv)) {
							$fdv = sprintf('%.3F', $fdv);
						}
						$s .= ' /'.$fdk.' '.$fdv.'';
		...
		protected function _putfontwidths($font, $cidoffset=0) {
			...
			$interval = false;
			// for each character
			foreach ((array) $font['cw'] as $cid => $width) {
				$cid -= $cidoffset;
				if ($width != $font['dw']) {
					if ($cid == ($prevcid + 1)) {
						// consecutive CID
		...
		protected function _puttruetypeunicode($font) {
			...
			foreach ((array) $font['desc'] as $key => $value) {
				if(is_float($value)) {
		...
		protected function _putcidfont0($font) {
			...
			if (isset($font['cidinfo']['uni2cid'])) {
				...
				$cw = array();
				foreach ((array) $font['cw'] as $uni => $width) {
			...
			$this->_newobj();
			$s = '<</Type /FontDescriptor /FontName /'.$name;
			foreach ((array) $font['desc'] as $k => $v) {
				if ($k != 'Style') {
					if(is_float($v)) {
						$v = sprintf('%.3F', $v);
					}
		...
		protected function _putimages() {
			$filter = ($this->compress) ? '/Filter /FlateDecode ' : '';
			foreach ((array) $this->imagekeys as $file) {
				$info = $this->getImageBuffer($file);
				$this->_newobj();
				...
				if (isset($info['trns']) AND is_array($info['trns'])) {
					$trns='';
					$count_info = count((array) $info['trns']);
		...
		protected function _putspotcolors() {
			foreach ((array) $this->spot_colors as $name => $color) {
				$this->_newobj();
		...
		protected function _putresourcedict() {
			...
			$out .= ' /Font <<';
			foreach ((array) $this->fontkeys as $fontkey) {
			...
			$out .= ' >>';
			// gradient patterns
			if (isset($this->gradients) AND (count((array) $this->gradients) > 0)) {
				$out .= ' /Pattern <<';
				foreach ((array) $this->gradients as $id => $grad) {
			...
			// spot colors
			if (isset($this->spot_colors) AND (count((array) $this->spot_colors) > 0)) {
				$out .= ' /ColorSpace <<';
				foreach ((array) $this->spot_colors as $color) {
		...
		protected function _putcatalog() {
			...
			$out .= ' >>';
			if (count((array) $this->outlines) > 0) {
			...
			if (!empty($this->form_obj_id) OR ($this->sign AND isset($this->signature_data['cert_type']))) {
				...
				if (isset($this->annotation_fonts) AND !empty($this->annotation_fonts)) {
					$out .= ' /DR <<';
					$out .= ' /Font <<';
					foreach ((array) $this->annotation_fonts as $font => $fontkey) {
		...
		protected function _putviewerpreferences() {
			...
			if (isset($this->viewer_preferences['PrintPageRange'])) {
				$PrintPageRangeNum = '';
				foreach ((array) $this->viewer_preferences['PrintPageRange'] as $k => $v) {
		...
		protected function _enddoc() {
			...
			// Embedded Files
			if (isset($this->embeddedfiles) AND count((array) $this->embeddedfiles) > 0) {
				$this->_out($this->embedded_start_obj_id.' '.count((array) $this->embeddedfiles));
				foreach ((array) $this->embeddedfiles as $filename => $filedata) {
			...
			if ($this->diskcache) {
				...
				foreach ((array) $this->imagekeys as $key) {
					// remove temporary files
					unlink($this->images[$key]);
				}
				foreach ((array) $this->fontkeys as $key) {
					// remove temporary files
		...
		protected function UTF8StringToArray($str) {
			...
			for ($i = 0; $i < $length; ++$i) {
				...
				if (count((array) $bytes) == 0) { // get starting octect
					if ($char <= 0x7F) {
		...
		public function Rect($x, $y, $w, $h, $style='', $border_style=array(), $fill_color=array()) {
			...
			if ($border_style) {
				$border_style2 = array();
				foreach ((array) $border_style as $line => $value) {
					$length = strlen($line);
					for ($i = 0; $i < $length; ++$i) {
		...
		public function Polygon($p, $style='', $line_style=array(), $fill_color=array(), $closed=true) {
			$nc = count((array) $p); // number of coordinates
		...
		protected function utf8Bidi($ta, $str='', $forcertl=false) {
			...
			for ($i=0; $i < $numchars; ++$i) {
				if ($ta[$i] == K_RLE) {
					...
					if (count((array) $remember)) {
						$last = count((array) $remember ) - 1;
			...
			if ($arabic) {
				...
				foreach ((array) $chardata2 as $key => $value) {
					if ($value['char'] === false) {
						unset($chardata2[$key]);
					}
				}
		...
		protected function _putbookmarks() {
			...
			$outline_p = array();
			$outline_y = array();
			foreach ((array) $this->outlines as $key => $row) {
				$outline_p[$key]  = $row['p'];
				$outline_k[$key] = $key;
			}
			...
			$lru = array();
			$level = 0;
			foreach ((array) $this->outlines as $i => $o) {
				if ($o['l'] > 0) {
					$parent = $lru[($o['l'] - 1)];
		...
		protected function _putjavascript() {
			...
			if (!empty($this->js_objects)) {
				foreach ((array) $this->js_objects as $key => $val) {
					if ($val['onload']) {
						$out .= ' (JS'.$key.') '.$key.' 0 R';
					}
				}
      ...
      foreach ($prop as $key => $val){
		...
		protected function getAnnotOptFromJSProp($prop) {
			...
			if (isset($prop['value'])) {
				if (is_array($prop['value'])) {
					$opt['opt'] = array();
					foreach ((array) $prop['value'] AS $key => $optval) {
		...
		protected function addExtGState($parms) {
			$n = count((array) $this->extgstates) + 1;
		...
		protected function _putextgstates() {
			$ne = count((array) $this->extgstates);
			for ($i = 1; $i <= $ne; ++$i) {
				$this->_newobj();
				$this->extgstates[$i]['n'] = $this->n;
				$out = '<< /Type /ExtGState';
				foreach ((array) $this->extgstates[$i]['parms'] as $k => $v) {
					if (is_float($v)) {
						$v = sprintf('%.2F', $v);
					}
					$out .= ' /'.$k.' '.$v;
		...
		public function colorRegistrationBar($x, $y, $w, $h, $transition=true, $vertical=false, $colors='A,R,G,B,C,M,Y,K') {
			$bars = explode(',', $colors);
			$numbars = count((array) $bars); // number of bars to print
			// set bar measures
			if ($vertical) {
		...
		public function cropMark($x, $y, $w, $h, $type='A,B,C,D', $color=array(0,0,0)) {
			...
			$crops = explode(',', $type);
			$numcrops = count((array) $crops); // number of crop marks to print
		...
		public function CoonsPatchMesh($x, $y, $w, $h, $col1=array(), $col2=array(), $col3=array(), $col4=array(), $coords=array(0.00,0.0,0.33,0.00,0.67,0.00,1.00,0.00,1.00,0.33,1.00,0.67,1.00,1.00,0.67,1.00,0.33,1.00,0.00,1.00,0.00,0.67,0.00,0.33), $coords_min=0, $coords_max=1, $antialias=false) {
			$this->Clip($x, $y, $w, $h);
			$n = count((array) $this->gradients) + 1;
			...
			for ($i=0; $i < $count_patch; ++$i) {
				...
				$count_cols = count((array) $patch_array[$i]['colors']);
		...
		public function Gradient($type, $coords, $stops, $background=array(), $antialias=false) {
			...
			$this->gradients[$n]['transparency'] = false;
			// color space
			$numcolspace = count((array) $stops[0]['color']);
			...
			$num_stops = count((array) $stops);
			$last_stop_id = $num_stops - 1;
			foreach ((array) $stops as $key => $stop) {
		...
		function _putshaders() {
			$idt = count((array) $this->gradients); //index for transparency gradients
			foreach ((array) $this->gradients as $id => $grad) {
				if (($grad['type'] == 2) OR ($grad['type'] == 3)) {
					...
					$encode = '';
					$i = 1;
					$num_cols = count((array) $grad['colors']);
					$lastcols = $num_cols - 1;
		...
		public function write1DBarcode($code, $type, $x='', $y='', $w='', $h='', $xres=0.4, $style='', $align='') {
			...
			// print bars
			if ($arrcode !== false) {
				foreach ((array) $arrcode['bcode'] as $k => $v) {
					$bw = ($v['w'] * $xres);
					if ($v['t']) {
						// draw a vertical bar
	...
	Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
              ...
              foreach ($style_array[1] as $id => $name){
							...
							if (isset($dom[$key]['style']['font-family'])) {
								...
								if (isset($dom[$key]['style']['font-family'])) {
									...
									foreach ((array) $fontslist as $font) {
		...
		public function writeHTML($html, $ln=true, $fill=false, $reseth=false, $cell=false, $align='') {
			...
			if (isset($this->PageAnnots[$this->page])) {
				$pask = count((array) $this->PageAnnots[$this->page]);
			} else {
				$pask = 0;
			}
			if (!$this->InFooter) {
				...
				if (($dom[$key]['value'] == 'tr') AND isset($dom[$key]['thead']) AND $dom[$key]['thead']) {
					if (isset($dom[$key]['parent']) AND isset($dom[$dom[$key]['parent']]['thead']) AND !$this->empty_string($dom[$dom[$key]['parent']]['thead'])) {
						...
						if (($this->start_transaction_page == ($this->numpages - 1)) OR ($this->y < $this->start_transaction_y) OR ($this->checkPageBreak($this->lasth, '', false))) {
							...
							// restore previous values
							foreach ((array) $this_method_vars as $vkey => $vval) {
								$$vkey = $vval;
							}
							// add a page (or trig AcceptPageBreak() for multicolumn mode)
							$pre_y = $this->y;
					...
					if ((!$this->newline) AND ($dom[$key]['value'] == 'img') AND (isset($dom[$key]['attribute']['height'])) AND ($dom[$key]['attribute']['height'] > 0)) {
						...
						if (!$autolinebreak) {
							...
							if ($this->page > $startlinepage) {
								...
								if (isset($this->PageAnnots[$this->page])) {
									$next_pask = count((array) $this->PageAnnots[$this->page]);
								} else {
									$next_pask = 0;
								}
								if (isset($this->PageAnnots[$startlinepage])) {
									foreach ((array) $this->PageAnnots[$startlinepage] as $pak => $pac) {
										if ($pak >= $pask) {
											$this->PageAnnots[$this->page][] = $pac;
											unset($this->PageAnnots[$startlinepage][$pak]);
											$npak = count((array) $this->PageAnnots[$this->page]) - 1;
						...
						if ((isset($plalign) AND ((($plalign == 'C') OR ($plalign == 'J') OR (($plalign == 'R') AND (!$this->rtl)) OR (($plalign == 'L') AND ($this->rtl))))) OR ($yshift < 0)) {
							...
							if ($plalign == 'C') {
								...
								if (preg_match_all('/\[\(([^\)]*)\)\]/x', $pmidtemp, $lnstring, PREG_PATTERN_ORDER)) {
									...
									$maxkk = count((array) $lnstring[1]) - 1;
									...
									while (preg_match('/([0-9\.\+\-]*)[\s](Td|cm|m|l|c|re)[\s]/x', $pmid, $strpiece, PREG_OFFSET_CAPTURE, $offset) == 1) {
										...
										if (isset($this->PageAnnots[$this->page])) {
											...
											foreach ((array) $this->PageAnnots[$this->page] as $pak => $pac) {
					...
					if ($dom[$key]['opening']) {
						...
						if (($dom[$key]['value'] == 'td') OR ($dom[$key]['value'] == 'th')) {
							...
							if (isset($dom[$table_el]['rowspans'])) {
								$rsk = 0;
								$rskmax = count((array) $dom[$table_el]['rowspans']);
								while ($rsk < $rskmax) {
							...
							if (isset($dom[$table_el]['rowspans'])) {
								...
								foreach ((array) $dom[$table_el]['rowspans'] as $k => $trwsp) {
									if ($trwsp['rowspan'] > 0) {
										if (isset($dom[$trid]['endpage'])) {
		...
		protected function closeHTMLTagHandler(&$dom, $key, $cell, $maxbottomliney=0) {
			...
			switch($tag['value']) {
				...
				case 'table': {
					...
					foreach ((array) $dom[($dom[$key]['parent'])]['trids'] as $j => $trkey) {
						// update row-spanned cells
						if (isset($dom[($dom[$key]['parent'])]['rowspans'])) {
							foreach ((array) $dom[($dom[$key]['parent'])]['rowspans'] as $k => $trwsp) {
								if ($trwsp['trid'] == $trkey) {
					...
					// for each row
					foreach ((array) $table_el['trids'] as $j => $trkey) {
						$parent = $dom[$trkey];
						// for each cell on the row
						foreach ((array) $parent['cellpos'] as $k => $cellpos) {
		...
		public function movePage($frompage, $topage) {
			...
			foreach ((array) $tmpoutlines as $key => $outline) {
			...
			// adjust links
			$tmplinks = $this->links;
			foreach ((array) $tmplinks as $key => $link) {
		...
		public function addTOC($page='', $numbersfont='', $filler='.', $toc_name='TOC') {
			...
			foreach ((array) $this->outlines as $key => $outline) {
				if ($this->rtl) {
					$aligntext = 'R';
					$alignnum = 'L';
				} else {
		...
		public function rollbackTransaction($self=false) {
			if (isset($this->objcopy)) {
				if (isset($this->objcopy->diskcache) AND $this->objcopy->diskcache) {
					...
					foreach ((array) $this->objcopy->cache_file_length as $file => $length) {
						$file = substr($file, 1);
						$handle = fopen($file, 'r+');
				...
				if ($self) {
					...
					foreach ((array) $objvars as $key => $value) {
						$this->$key = $value;
					}
				}
				return $this->objcopy;
		...
		public function setColumnsArray($columns) {
			...
			$this->num_columns = count((array) $columns);
			$this->current_column = 0;
			$this->column_start_page = $this->page;
		}
			...
			for ($pos = 0; $pos < $maxpos; ++$pos) {
				...
				for ($i = $charmin; $i <= $imax; ++$i) {
					...
					if (isset($patterns[$subword])) {
						...
						$pattern_length = count((array) $pattern);
						$digits = 1;
		...
		public function hyphenateText($text, $patterns, $dictionary=array(), $leftmin=1, $rightmin=2, $charmin=1, $charmax=8) {
			...
			// for each char
			foreach ((array) $unichars as $char) {
			...
			if (isset($regs[1]) AND !empty($regs[1])) {
				...
				if (preg_match('/[\s]+viewBox[\s]*=[\s]*"[\s]*([0-9\.\-]+)[\s]+([0-9\.\-]+)[\s]+([0-9\.]+)[\s]+([0-9\.]+)[\s]*"/si', $regs[1], $tmp)) {
					...
					if (preg_match('/[\s]+preserveAspectRatio[\s]*=[\s]*"([^"]*)"/si', $regs[1], $tmp)) {
						...
						switch (count((array) $aspect_ratio)) {
							case 3: {
		...
		protected function setSVGStyles($svgstyle, $prevsvgstyle, $x=0, $y=0, $w=1, $h=1, $clip_function='', $clip_params=array())  {
			...
			if (preg_match('/url\([\s]*\#([^\)]*)\)/si', $svgstyle['fill'], $regs)) {
				...
				if (!empty($clip_function) AND method_exists($this, $clip_function)) {
					...
					if (is_array($bbox) AND (count((array) $bbox) == 4)) {
						list($x, $y, $w, $h) = $bbox;
					}
				}
				...
				if (count((array) $gradient['stops']) > 1) {
		...
		protected function SVGPath($d, $style='') {
			...
			$relcoord = false;
			// draw curve pieces
			foreach ((array) $paths as $key => $val) {
				// get curve type
				$cmd = trim($val[1]);
				...
				if (isset($val[2])) {
					...
					$params = array();
					foreach ((array) $rawparams as $ck => $cp) {
						$params[$ck] = $this->getHTMLUnitToUnits($cp, 0, $this->svgunit, false);
					}
				}
				switch (strtoupper($cmd)) {
					case 'M': { // moveto
						foreach ((array) $params as $ck => $cp) {
							if (($ck % 2) == 0) {
								$x = $cp + $xoffset;
							} else {
								$y = $cp + $yoffset;
		...
		protected function startSVGElementHandler($parser, $name, $attribs) {
			...
			$prev_svgstyle = $this->svgstyles[(count((array) $this->svgstyles) - 1)]; // previous style
			$svgstyle = array(); // current style
			if (isset($attribs['style'])) {
				$attribs['style'] = ';'.$attribs['style'];
			}
			foreach ((array) $prev_svgstyle as $key => $val) {
			...
			$tm = $this->svgstyles[(count((array) $this->svgstyles) - 1)]['transfmatrix'];
			...
			switch($name) {
				...
				case 'polygon': {
					...
					$ymin = 2147483647;
					$ymax = 0;
					foreach ((array) $points as $key => $val) {
